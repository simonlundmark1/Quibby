...
const voteTracker = new Map<string, Map<string, string>>();

/**
 * Record a vote for a specific user on a specific question
 */
export function recordVote(questionId: string, userId: string, answerId: string): void {
  try {
    ...
    if (!voteTracker.has(questionId)) {
      voteTracker.set(questionId, new Map<string, string>());
    }
 ...
 */
export function hasVoted(questionId: string, userId: string): boolean {
  try {
 ...
 */
export function getVotedAnswerId(questionId: string, userId: string): string | null {
  try {
    if (!questionId || !userId) {
 ...
 */
export function getVotesForQuestion(questionId: string): Map<string, string> | null {
  try {
    if (!questionId) {
